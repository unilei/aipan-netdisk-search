...
- 服务器：通过 GitHub Actions Secret `DEPLOY_SERVER_HOST` 配置
...
- 如果你想用别的镜像仓库名，需要改 [`deploy.yml`](../.github/workflows/deploy.yml)
...
- 这个用户能登录 `DEPLOY_SERVER_HOST` 指向的服务器
- 这个用户能执行 Docker
...
### 5. `DEPLOY_SERVER_HOST`

用途：

- GitHub Actions SSH 登录生产应用服务器的主机名或 IP

如何获取：

- 使用当前生产服务器的公网地址或可解析域名
...
- 当前工作流默认 SSH 端口是 `22`
- 如果你的服务器不是 `22`，需要修改 [`deploy.yml`](../.github/workflows/deploy.yml)
...
gh secret set DEPLOY_SERVER_HOST -b"<APP_SERVER_HOST>"
...
gh variable set NUXT_PUBLIC_GITHUB_BRANCH -b"master"
```
