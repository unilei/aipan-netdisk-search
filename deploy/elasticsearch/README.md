...
- ES VPS：`<ELASTICSEARCH_HOST>`
- 应用服务器白名单 IP：`<APP_SERVER_EGRESS_IP>`
- ES 端口：`9200`
...
`9200` 只允许应用服务器出口 IP 访问。

示例 `ufw`：

```bash
sudo ufw allow from <APP_SERVER_EGRESS_IP> to any port 9200 proto tcp
sudo ufw deny 9200/tcp
sudo ufw reload
sudo ufw status
```

如果使用云厂商安全组，也需要在安全组里只放行 `<APP_SERVER_EGRESS_IP>/32 -> 9200`。
...
- Secrets：`ELASTICSEARCH_NODE`、`ELASTICSEARCH_USERNAME`、`ELASTICSEARCH_PASSWORD`
- Variables：`ELASTICSEARCH_CA_FINGERPRINT`、`ELASTICSEARCH_USER_RESOURCE_INDEX`
