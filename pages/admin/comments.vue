# 评论管理页面
<script setup>
import { House, ChatLineRound, Delete, ArrowLeft, Search } from '@element-plus/icons-vue'
        ...
        <div class="max-w-[1240px] mx-auto space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <div class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                            ...
                            <span>/</span>
                            <span class="text-gray-900">评论管理</span>
                        </div>
                        <h1 class="text-2xl font-bold text-gray-900">评论管理</h1>
                        <p class="text-gray-500 mt-1">管理和维护博客评论内容</p>
                    </div>
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm">
                <div class="flex items-center space-x-4">
                    <el-select v-model="searchForm.searchType" placeholder="搜索类型" class="!w-32">
                        <el-option v-for="option in searchTypeOptions" :key="option.value" :label="option.label"
                            :value="option.value" />
                    </el-select>
                    <el-input v-model="searchForm.keyword" placeholder="请输入搜索关键词" class="!w-80" clearable
            ...
            <!-- 表格区域 -->
            <div class="bg-white rounded-lg p-6 shadow-sm">
                <el-table v-loading="loading" :data="comments" style="width: 100%" :border="true" class="mt-4">
                    <el-table-column type="index" label="序号" width="80" align="center" />
                    <el-table-column label="评论者" width="200">
                        <template #default="{ row }">
                            <div class="flex items-center">
                                <img :src="row.avatar" :alt="row.author" class="w-8 h-8 rounded-full mr-2">
                                <div>
                    ...
                    <el-table-column label="操作" width="120" fixed="right" align="center">
                        <template #default="{ row }">
                            <el-popconfirm title="确定要删除这条评论吗？这将同时删除所有回复。" @confirm="handleDeleteComment(row)">
                                <template #reference>
                ...
                <!-- 分页器 -->
                <div class="mt-6 flex justify-center" v-if="!loading && comments.length > 0">
