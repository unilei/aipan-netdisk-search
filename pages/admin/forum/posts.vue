<template>
    <div class="min-h-[calc(100vh-60px)] bg-gray-50 p-6">
        <div class="max-w-[1240px] mx-auto space-y-6">
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <div class="w-64">
                        <el-select v-model="filters.topicId" placeholder="选择主题" clearable class="w-full">
                            <el-option label="全部主题" value="" />
                            <el-option v-for="topic in topics" :key="topic.id" :label="topic.title" :value="topic.id" />
                        </el-select>
                    </div>
                    <div class="flex-1">
                        <el-input v-model="filters.keyword" placeholder="搜索回复内容" clearable class="w-full"
                            @keyup.enter="loadPosts" />
                    </div>
                    <el-button type="primary" @click="loadPosts" class="flex items-center">
                        <el-icon class="mr-1">
                            <Search />
                        </el-icon>
                        搜索
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm">
                ...
                <div v-else>
                    <el-table :data="posts" style="width: 100%" border>
                        ...
                        <el-table-column prop="topic.title" label="所属主题" min-width="200" show-overflow-tooltip>
                            <template #default="scope">
                                <a @click="navigateToTopic(scope.row.topicId)"
                        ...
                        <el-table-column label="操作" width="250" fixed="right">
                            <template #default="scope">
                                <div class="flex space-x-2">
                                    ...
                                    <el-button size="small" type="primary" @click="editPost(scope.row)">
                                        <el-icon class="mr-1">
                                            <Edit />
                                        </el-icon>
                                        编辑
        ...
        <!-- 编辑回复对话框 -->
        <el-dialog v-model="editDialogVisible" title="编辑回复" width="700px" destroy-on-close>
            <el-form v-if="form" :model="form" label-width="80px" :rules="rules" ref="formRef">
                <el-form-item label="主题" prop="topicId">
                    <el-select v-model="form.topicId" placeholder="选择主题" class="w-full" disabled>
                        <el-option v-for="topic in topics" :key="topic.id" :label="topic.title" :value="topic.id" />
                    </el-select>
                </el-form-item>
                <el-form-item label="内容" prop="content">
                    <el-input v-model="form.content" type="textarea" :rows="6" placeholder="请输入回复内容" />
                </el-form-item>
...
import {
    House, Search, Refresh, ArrowLeft, Menu, Document, Edit, Delete,
    Check, Close
...
async function deletePost() {
    ...
    try {
        const token = useCookie('token').value
        const response = await $fetch(`/api/admin/forum/posts/${selectedPost.value.id}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${token}`
            }
