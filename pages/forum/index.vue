<template>
    <div class="bg-gray-50 dark:bg-gray-900 min-h-screen pb-12">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-blue-600 text-white">
            <div class="max-w-6xl mx-auto p-4">
                <div class="max-w-4xl">
                    ...
                    <p class="text-sm text-blue-100 mb-4">分享想法、提问、参与讨论，连接社区中的每一个人</p>
        ...
        <div class="max-w-6xl mx-auto p-4">
            <!-- 统计信息卡片 -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg mb-4 p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="flex items-center p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                        <div
                            class="w-8 h-8 flex items-center justify-center bg-purple-100 dark:bg-purple-900 rounded-full mr-4">
                            <i class="text-sm fas fa-comments text-purple-600 dark:text-purple-400"></i>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">板块数量</div>
                            <div class="text-xs font-bold">{{ categories?.length || 0 }}</div>
                        </div>
                    </div>
                    <div class="flex items-center p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                        <div
                            class="w-8 h-8 flex items-center justify-center bg-blue-100 dark:bg-blue-900 rounded-full mr-4">
                            <i class="text-sm fas fa-file-alt text-blue-600 dark:text-blue-400"></i>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">主题数量</div>
                            <div class="text-xs font-bold">{{ totalTopics }}</div>
                        </div>
                    </div>
                    <div class="flex items-center p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                        <div
                            class="w-8 h-8 flex items-center justify-center bg-purple-100 dark:bg-purple-900 rounded-full mr-4">
                            <i class="text-sm fas fa-users text-purple-600 dark:text-purple-400"></i>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">社区成员</div>
                            <div class="text-xs font-bold">{{ user ? '已登录' : '未登录' }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="!categories || categories.length === 0"
                ...
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">暂无论坛分类</h3>
                    ...
                    <div v-for="category in categories" :key="category.id"
                        ...
                        <div class="p-5 flex items-start flex-1">
                            <div v-if="category.icon"
                                class="w-8 h-8 flex-shrink-0 bg-purple-100 dark:bg-purple-900 rounded-lg flex items-center justify-center mr-4">
                                <i :class="[category.icon, 'text-purple-600 dark:text-purple-400 text-sm']"></i>
                            </div>
                            <div v-else
                                class="w-8 h-8 flex-shrink-0 bg-purple-100 dark:bg-purple-900 rounded-lg flex items-center justify-center mr-4">
                            ...
                            <div class="flex-1">
                                <h3 class="text-sm font-semibold mb-2 text-gray-800 dark:text-white">
                                    {{ category.name }}
                                </h3>
                        ...
                        <div
                            ...
                            <NuxtLink :to="`/forum/category/${category.slug}`"
                                class="text-xs text-purple-600 dark:text-purple-400 font-medium hover:text-purple-800 dark:hover:text-purple-300 flex items-center justify-between">
                                进入板块
