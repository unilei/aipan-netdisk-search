...
// SEO配置
useHead({
    title: computed(() => blog.value?.title ? `${blog.value.title} | AIPAN.ME 博客` : 'AIPAN.ME 博客'),
    meta: [
        {
            name: 'description',
            content: computed(() => {
        ...
        {
            property: 'og:title',
            content: computed(() => blog.value?.title ? `${blog.value.title} | AIPAN.ME 博客` : 'AIPAN.ME 博客')
        },
        {
        ...
        { name: 'author', content: computed(() => blog.value?.author || 'AIPAN.ME') },
        {
            name: 'article:published_time',
            content: computed(() => blog.value?.createdAt || new Date().toISOString())
        },
        {
            name: 'article:modified_time',
            content: computed(() => blog.value?.updatedAt || blog.value?.createdAt || new Date().toISOString())
        },
        {
            name: 'article:section',
            content: computed(() => blog.value?.categories?.[0]?.category.name || '未分类')
        }
    ],
    link: [
        {
...
// 监听内容变化
watch([() => parsedContent.value, () => mounted.value], () => {
    if (mounted.value) {
        initToc();
    }
}, { immediate: true });
