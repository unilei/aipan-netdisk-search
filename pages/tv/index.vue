...
<template>
  <div
    ...
    <div ref="tvContainer" class="relative w-full max-w-6xl mx-auto px-8 tv-container"
      ...
      <!-- TV Frame -->
      <div class="tv-frame rounded-[2.5rem] shadow-2xl p-8 transform transition-all duration-300 retro-tv-frame">
        <!-- TV Screen -->
          ...
          <div
            ...
            <div
              ...
              <button class="control-btn" @click="handleMute">
                <span class="flex items-center gap-2">
                  <i :class="!videoMuted ? 'fas fa-volume-up' : 'fas fa-volume-mute'
                    "></i>
            ...
            <div class="tv-decoration flex items-center justify-center my-2 md:my-0">
              <button class="control-power-btn" :class="{ 'power-on': videoPlayStatus }" @click="handlePowerSwitch">
    ...
    <div v-if="modalShow" class="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center">
      <div
        ...
        <div class="tv-channel-sidebar">
          <button class="tv-btn danger rounded-md p-2 w-full" @click="modalShow = false">
            关闭
          </button>
          <div class="mt-4 space-y-2">
            <button v-for="category in channelCategoryData" :key="category.id" class="tv-category-btn w-full"
        ...
        <div class="tv-channel-content">
          <div class="channel-search">
            ...
            <button class="tv-btn primary p-2 text-sm rounded-md" @click="handleSwitchSource(videoSrc)">
              切换视频
            </button>
          </div>
          ...
          <div class="space-y-6">
            <div v-if="channelCategory === 1" class="space-y-3">
              <div class="channel-item" :class="{ active: item.url === videoSrc }" v-for="(item, index) in tvSources"
                :key="index" @click="handleSwitchSource(item.url)">
            ...
            <div v-if="channelCategory === 2">
              <div class="source-categories">
                <div class="source-category" :class="{ active: sourceIndex === index }"
              ...
              <div class="mt-6 space-y-3">
                <div class="channel-item" :class="{ active: item.url === videoSrc }"
            ...
            <div class="space-y-3" v-if="channelCategory === 3">
              <div class="alist-nav">
                <button class="alist-nav-btn back" v-if="
                  alistPath.length > 1 ||
                ...
                <button class="alist-nav-btn home" type="button" @click="handleHomeAlist()">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path
                ...
                <button class="alist-nav-btn settings" :class="{ active: alistSettingShow }" type="button"
              ...
              <div class="space-y-3" v-if="alistSettingShow">
                <div class="alist-item setting-item" :class="{ active: alistUrl === item.link }"
                  v-for="(item, index) in alistSettingData" :key="index" @click="handleClickAlistUrl(item)">
              ...
              <div class="space-y-3" v-else>
                <div class="alist-item" :class="{
                  'folder-item': item.is_dir,
                  'file-item': !item.is_dir,
                  active: !item.is_dir && alistCurrentPlayIndex === index,
                }" v-for="(item, index) in alistData?.content" :key="index" @click="handleClickAlist(item, index)">
                  <div class="flex items-center space-x-3">
                    <!-- Folder Icon -->
                    <svg v-if="item.is_dir" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                      fill="currentColor">
...
.glass-effect {
  @apply bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 hover:bg-gray-700/50 px-4 py-2 rounded-lg text-gray-200 transition-all duration-300;
}
...
.tv-screen::before {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(rgba(255, 255, 255, 0.1) 50%,
      rgba(0, 0, 0, 0.1) 50%);
...
/* Control Buttons */
.control-btn {
  @apply px-3 md:px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 bg-neutral-800/80 hover:bg-neutral-700/80 text-neutral-300 hover:text-white border border-neutral-600/30 hover:border-neutral-500/30 shadow-lg hover:shadow-xl min-w-[40px] md:min-w-[auto];
  backdrop-filter: blur(8px);
}

.control-btn.primary {
  @apply bg-blue-600/90 hover:bg-blue-500/90 text-blue-100 hover:text-white border-blue-500/30 hover:border-blue-400/30;
}
...
.tv-channel-panel {
  @apply fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center;
}

.tv-channel-dialog {
  @apply w-[90%] max-w-6xl h-[90vh] bg-white/5 rounded-2xl flex overflow-hidden shadow-2xl border border-white/10;
...
.tv-category-btn {
  @apply mb-3 p-3 text-sm font-medium rounded-xl transition-all flex flex-col items-center justify-center gap-2 text-white/90 hover:text-white hover:bg-white/10 active:bg-white/15;
...
.tv-input {
  @apply flex-1 bg-white/10 text-white px-5 py-3 rounded-xl border border-white/10 focus:border-blue-400 focus:outline-none transition-all duration-200 placeholder:text-gray-300;
...
.tv-btn.primary {
  @apply bg-blue-500 hover:bg-blue-600 text-white transition-all duration-200 shadow-lg border border-blue-400/30;
...
.tv-btn.danger {
  @apply bg-red-500 hover:bg-red-600 text-white transition-all duration-200 shadow-lg border border-red-400/30 hover:border-red-500/30;
...
.channel-item {
  @apply p-4 rounded-xl border transition-all duration-200 cursor-pointer text-white font-medium;
  background: rgba(255, 255, 255, 0.05);
  border-color: rgba(255, 255, 255, 0.1);
}
...
.channel-item {
  @apply p-4 rounded-xl transition-all duration-200 cursor-pointer bg-white/10 hover:bg-white/20 border border-white/10 hover:border-white/30 relative overflow-hidden;
}
...
.channel-number {
  @apply w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-blue-300 font-bold text-sm mr-4;
}
...
.source-category {
  @apply px-4 py-2 rounded-lg cursor-pointer bg-white/10 hover:bg-white/20 border border-white/10 hover:border-white/30 text-white/90 hover:text-white font-medium text-sm transition-all duration-200;
}

.source-category.active {
  @apply bg-blue-500 border-blue-400 text-white font-semibold;
...
/* Alist Item Styles */
.alist-item {
  @apply p-4 rounded-xl transition-all duration-200 cursor-pointer bg-white/10 hover:bg-white/15 border border-white/10 text-white relative overflow-hidden;
}
...
.alist-item.folder-item {
  @apply bg-yellow-500/10 hover:bg-yellow-500/20 border-yellow-500/20 hover:border-yellow-500/30;
}
...
.alist-item.file-item {
  @apply bg-blue-500/10 hover:bg-blue-500/20 border-blue-500/20 hover:border-blue-500/30;
}
...
.alist-item.setting-item {
  @apply bg-purple-500/10 hover:bg-purple-500/20 border-purple-500/20 hover:border-purple-500/30;
}
...
.alist-nav-btn {
  @apply flex items-center space-x-2 px-4 py-2 rounded-xl transition-all duration-200 font-medium text-sm border border-white/10 hover:scale-105 hover:-translate-y-0.5;
}
...
.alist-nav-btn.back {
  @apply bg-gray-500/20 hover:bg-gray-500/30 text-gray-200 hover:text-white;
}
...
.alist-nav-btn.home {
  @apply bg-blue-500/20 hover:bg-blue-500/30 text-blue-200 hover:text-white;
}
...
.alist-nav-btn.settings {
  @apply bg-purple-500/20 hover:bg-purple-500/30 text-purple-200 hover:text-white;
}
